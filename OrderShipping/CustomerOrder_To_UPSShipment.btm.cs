namespace OrderShipping {
    ...
    public sealed class CustomerOrder_To_UPSShipment : global::Microsoft.XLANGs.BaseTypes.TransformBase {
        
        private const string _strMap = @"<?xml version=""1.0"" encoding=""UTF-16""?>
<xsl:stylesheet xmlns:xsl=""http://www.w3.org/1999/XSL/Transform"" xmlns:msxsl=""urn:schemas-microsoft-com:xslt"" xmlns:var=""http://schemas.microsoft.com/BizTalk/2003/var"" exclude-result-prefixes=""msxsl var s0 userCSharp"" version=""1.0"" xmlns:s0=""http://OrderShipping.Orders"" xmlns:ns0=""http://OrderShipping.UPSShipment"" xmlns:userCSharp=""http://schemas.microsoft.com/BizTalk/2003/userCSharp"">
  ...
  <xsl:template match=""/s0:Order"">
    ...
    <xsl:variable name=""var:v2"" select=""userCSharp:StringConcat(&quot;Texas County&quot;)"" />
    <ns0:UPSShipment>
      <xsl:attribute name=""DeliveryName"">
        <xsl:value-of select=""$var:v1"" />
      </xsl:attribute>
      <xsl:if test=""ShipToAddress"">
        <xsl:attribute name=""DeliveryAddress1"">
          <xsl:value-of select=""ShipToAddress/text()"" />
        </xsl:attribute>
      </xsl:if>
      <xsl:if test=""ShipToCity"">
        <xsl:attribute name=""DeliveryCity"">
          <xsl:value-of select=""ShipToCity/text()"" />
        </xsl:attribute>
      </xsl:if>
      <xsl:if test=""ShipToState"">
        <xsl:attribute name=""DeliveryState"">
          <xsl:value-of select=""ShipToState/text()"" />
        </xsl:attribute>
      </xsl:if>
      <xsl:attribute name=""DeliveryPostalCode"">
        <xsl:value-of select=""ShipToZip/text()"" />
      </xsl:attribute>
      <xsl:attribute name=""DeliveryCountry"">
        <xsl:value-of select=""$var:v2"" />
      </xsl:attribute>
      <xsl:variable name=""var:v3"" select=""userCSharp:InitCumulativeSum(0)"" />
      <xsl:for-each select=""/s0:Order/LineItems/LineItem"">
        <xsl:variable name=""var:v4"" select=""userCSharp:MathMultiply(string(Quantity/text()) , string(Price/text()))"" />
      ...
      <xsl:attribute name=""InsureAmount"">
